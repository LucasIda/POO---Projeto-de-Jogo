...
public static class HandValue
{
    private class HandScoreData
    {
        ...
        // Precisa ser alterada futuramente
        public int GetScore() => Chips * Multiplier;
    }

    // Tabela de valores para cada mão
    ...
    {
        return HandTable.ContainsKey(hand) ? HandTable[hand].Chips : 0;
    }

    // Retorna multiplicador
    public static int GetMultiplier(HandChecker.HandType hand)
    {
        return HandTable.ContainsKey(hand) ? HandTable[hand].Multiplier : 0;
    }

    // Retorna score (chips * mult)
    public static int GetScore(HandChecker.HandType hand)
    {
        return HandTable.ContainsKey(hand) ? HandTable[hand].GetScore() : 0;
    }
}
